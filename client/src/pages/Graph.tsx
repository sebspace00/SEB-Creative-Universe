...
export default function Graph() {
  ...
  const { data: connectionsData } = trpc.connections.list.useQuery();
  ...
  const buildGraph = useCallback(() => {
    ...
    const simulation = d3.forceSimulation<NodeDatum>(allNodes)
      .force("link", d3.forceLink<NodeDatum, LinkDatum>(links).id(d => d.id).distance(90).strength(0.4))
    ...
    const link = g.append("g").selectAll("line").data(links).join("line")
      .attr("stroke", d => {
        const c: Record<string, string> = { theme: "rgba(0,212,255,0.25)", symbol: "rgba(192,132,252,0.25)", emotion: "rgba(240,192,64,0.25)", narrative: "rgba(155,89,182,0.25)", character: "rgba(255,107,107,0.25)" };
        return c[d.connectionType] ?? "rgba(255,255,255,0.1)";
      })
    ...
    node.append("circle")
      .attr("r", 6)
      .attr("fill", d => DISTRICT_COLORS[d.districtId ?? 0] ?? "#888")
    ...
    node
      .on("mouseenter", (event, d) => {
        d3.select(event.currentTarget).select("circle").attr("r", 10).attr("fill-opacity", 1);
        const rect = svgRef.current?.getBoundingClientRect();
        if (rect) setTooltip({ x: event.clientX - rect.left, y: event.clientY - rect.top, node: d });
      })
      .on("mouseleave", (event) => {
        d3.select(event.currentTarget).select("circle").attr("r", 6).attr("fill-opacity", 0.85);
        setTooltip(null);
      })
        ...
        <div className="absolute bottom-4 left-4 p-3 rounded-xl text-[10px]"
          ...
          {[1, 2, 3, 4].map(id => (
            <div key={id} className="flex items-center gap-2 mb-1">
              <div className="w-2 h-2 rounded-full" style={{ background: DISTRICT_COLORS[id] }} />
